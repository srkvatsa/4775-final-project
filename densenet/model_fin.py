...
def main():
    ...
    parser.add_argument('--train', dest='TRAIN', help="Training dataset in .npy", default=None)
    parser.add_argument('--valid', dest='VALID', help="Validation dataset in .npy", default=None)
    parser.add_argument('--test', dest='TEST', help="Test dataset in .npy", default=None)
    parser.add_argument('--convert_dataset', dest='convert_dataset', type=int, help="flag to convert raw FASTA into npy", default=0)
    parser.add_argument('--gapped', dest='gapped', type=int, help="flag to determine whether to use gapped or ungapped dataset", required=True)
    ...
    if args.convert_dataset:
        ...
        train_aln = os.path.join(data_path, "TRAIN.aln")
        valid_aln = os.path.join(data_path, "VALID.aln")
        test_aln  = os.path.join(data_path, "TEST.aln")
        if not (os.path.exists(train_aln) and os.path.exists(valid_aln) and os.path.exists(test_aln)):
            print("Error: One or more alignment files (.aln) do not exist.")
            return
        train_data, valid_data, test_data = tv_parse(train_aln, valid_aln, test_aln)
        ...
        # If --train, --valid, --test are not provided, set them based on --gapped flag
        if args.TRAIN is None:
            args.TRAIN = os.path.join(data_path, "TRAIN.npy")
        if args.VALID is None:
            args.VALID = os.path.join(data_path, "VALID.npy")
        if args.TEST is None:
            args.TEST = os.path.join(data_path, "TEST.npy")
        
        # Check if the files exist
        if not (os.path.exists(args.TRAIN) and os.path.exists(args.VALID) and os.path.exists(args.TEST)):
            print("Error: One or more .npy files do not exist. Please provide correct paths or set --convert_dataset to 1.")
            return
